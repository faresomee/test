...
session_start();
#We use session to store information (in variables) to be used across multiple pages
...
if(isset($_POST["email"]) && isset($_POST["password"])){
	#mysqli_real_escape_string -> تعيد سترنغ وتستخدم للتحقق من وجود المطلوب في قاعدة البيانات ومستوى الامان عالي اذا لم يكون في قاعدة البيانات تعيد لاشيء
	$email = mysqli_real_escape_string($con,$_POST["email"]);
	$password = $_POST["password"];
	$sql = "SELECT * FROM user_info WHERE email = '$email' AND password = '$password'";
	$run_query = mysqli_query($con,$sql);
	#mysqli_num_rows & mysqli_fetch_array ->FOR QUERY      
	$count = mysqli_num_rows($run_query);
    $row = mysqli_fetch_array($run_query);
		$_SESSION["uid"] = $row["user_id"];
		$_SESSION["name"] = $row["first_name"];
		$ip_add = getenv("REMOTE_ADDR"); #return ip address
	...
	if($count == 1){
			...
			#for admin
                $email = mysqli_real_escape_string($con,$_POST["email"]); #التاكد من الوجود في قاعدة البيانات مع امان عالي
                $password =($_POST["password"]) ;
